...
export default function AddBatchPage() {
  ...
  const addBatchMutation = useMutation({
    mutationFn: async (newBatch: Partial<Batch>) => {
      ...
      await fetch('/api/products', {
        ...
        body: JSON.stringify({
          id: productId,
          totalStock: newBatch.initialQuantity
        }),
      });
      return createdBatch;
    },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isNaN(initialQuantity) || initialQuantity <= 0) throw new Error(t('batches.enterValidQuantity'));
      if (isNaN(purchasePrice) || purchasePrice <= 0) throw new Error(t('batches.enterValidPrice'));
