...
export default function BatchHistoryPage() {
  ...
  const exportCSV = () => {
    ...
    const rows = filteredBatches.map(batch => [
      ...
      batch.productName,
      batch.category || t('inventory.uncategorized'),
  ...
  const formatExpirationDate = (dateString: string | null) => {
    if (!dateString) return t('inventory.notSet');
      ...
      <div className="flex-1 overflow-auto bg-gray-50 dark:bg-slate-900">
        <div className="p-3 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 gap-3">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">{t('batches.batchHistory')}</h1>
              <p className="text-sm text-gray-600 dark:text-gray-300">
                {t('batches.viewing')} {new Date(dateRange.start).toLocaleDateString()} {t('common.to')} {new Date(dateRange.end).toLocaleDateString()}
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
            <div className="bg-white dark:bg-slate-800 p-3 sm:p-4 rounded-lg border border-gray-200 dark:border-slate-700 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">{t('batches.totalBatches')}</p>
            ...
            <div className="bg-white dark:bg-slate-800 p-3 sm:p-4 rounded-lg border border-gray-200 dark:border-slate-700 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">{t('batches.initialUnits')}</p>
            ...
            <div className="bg-white dark:bg-slate-800 p-3 sm:p-4 rounded-lg border border-gray-200 dark:border-slate-700 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">{t('batches.currentStock')}</p>
            ...
            <div className="bg-white dark:bg-slate-800 p-3 sm:p-4 rounded-lg border border-gray-200 dark:border-slate-700 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">{t('batches.totalSpent')}</p>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 mb-4 sm:mb-6">
            <div className="p-3 sm:p-4 border-b border-gray-200 dark:border-slate-700">
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                ...
                <div className="grid grid-cols-2 gap-2 sm:flex-1 sm:flex sm:gap-4">
                  <div className="flex flex-col">
                    <label className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('batches.startDate')}</label>
                  ...
                  <div className="flex flex-col">
                    <label className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t('batches.endDate')}</label>
                    <div className="relative">
                      <div className="absolute inset-y-0 left-0 flex items-center pl-2 pointer-events-none">
                        <Calendar size={16} className="text-gray-500 dark:text-gray-400" />
                      </div>
                ...
                <div className="flex-1 sm:max-w-[200px]">
                  <label className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">{t('common.status')}</label>
                  <select
                    value={statusFilter}
                  ...
                  >
                    <option value="all">{t('batches.allBatches')}</option>
                    <option value="active">{t('batches.activeOnly')}</option>
                    <option value="depleted">{t('batches.depletedOnly')}</option>
                  </select>
                </div>
              ...
              <div className="mt-3 flex justify-between items-center text-sm text-gray-600 dark:text-gray-300">
                <span>{t('batches.foundCount', { count: filteredBatches.length })}</span>
                <div className="flex items-center gap-2">
                  <span>{t('batches.sortBy')}:</span>
                  <button 
            ...
            {isLoading ? (
              <div className="p-8 text-center">
                ...
                <p className="mt-2 text-gray-600 dark:text-gray-400">{t('batches.loading')}</p>
              </div>
            ) : error ? (
              <div className="p-8 text-center">
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-gray-600 dark:text-gray-300">{t('batches.noResults')}</p>
                      ...
                      {currentBatches.map((batch) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2.5 max-w-[120px] truncate text-sm text-gray-900 dark:text-gray-100">
                            {batch.supplier || t('batches.noSupplier')}
                          </td>
                          ...
                          <td className="px-3 py-2.5 whitespace-nowrap text-sm">
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              ...
                              {batch.status === 'active' ? t('common.active') : t('common.depleted')}
                            </span>
                          </td>
                        </tr>
                      ))}
                      ...
                      <div className="p-4">
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">{batch.supplier || t('batches.noSupplier')}</p>
                          </div>
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            batch.status === 'active' 
                              ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400'
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('batches.purchaseDate')}</p>
                          ...
                          <div>
                            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('common.price')}</p>
                            <p className="text-sm text-gray-900 dark:text-gray-100">${batch.purchasePrice.toFixed(2)}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('batches.initial')}</p>
                            <p className="text-sm text-gray-900 dark:text-gray-100">{batch.initialQuantity} {t('inventory.units')}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('batches.current')}</p>
                            <p className="text-sm text-gray-900 dark:text-gray-100">{batch.currentQuantity} {t('inventory.units')}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                      <div>
                        <p className="text-sm text-gray-700 dark:text-gray-300">
                          {t('common.showing')} <span className="font-medium">{(currentPage - 1) * itemsPerPage + 1}</span> {t('common.to')} <span className="font-medium">{Math.min(currentPage * itemsPerPage, filteredBatches.length)}</span> {t('common.of')} <span className="font-medium">{filteredBatches.length}</span> {t('batches.batchesFound')}
                        </p>
                      </div>
                      <div>
                        <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                          ...
                          {/* Page number */}
                          <span className="relative inline-flex items-center px-4 py-2 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm font-medium text-gray-700 dark:text-gray-200">
                            {currentPage}
                          </span>
                          <button
                            onClick={() => changePage(currentPage < totalPages ? currentPage + 1 : totalPages)}
